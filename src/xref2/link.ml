...
let rec unfold_alias env (m : Paths.Path.Module.t) : Paths.Path.Module.t option
    =
  let alias_target =
  ...
  | None -> (
      match m with
      | `Dot (parent, name) ->
          unfold_alias env parent |> Opt.map (fun parent -> `Dot (parent, name))
      | _ -> None)
...
and instance_prefix env (m : Paths.Path.Module.t) : Paths.Path.Module.t option =
  match unfold_alias env m with
  | Some unfolded when is_instance unfolded ->
      Some (resolve_module_path_parts env unfolded)
  | _ -> None
...
let warn_on_hidden_representation (id : Id.Type.t)
  ...
  let rec internal_typ_exp typ_expr =
    ...
    | Arrow (_, (t, _), (t2, _)) -> internal_typ_exp t || internal_typ_exp t2
...
and type_expression : Env.t -> Id.Signature.t -> _ -> _ =
 fun env parent visited texpr ->
  ...
  | Arrow (lbl, (t1, m1), (t2, m2)) ->
      Arrow
        ( lbl,
          (type_expression env parent visited t1, m1),
          (type_expression env parent visited t2, m2) )
  | Tuple ts ->
      Tuple
        (List.map
